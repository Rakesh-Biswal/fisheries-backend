...
const MeetingAttendance = require("../../models/TEAMLEADER/MeetingAtte");
const TeamLeaderEmployee = require("../../models/TEAMLEADER/TeamLeaderEmployee");
const SalesEmployee = require("../../models/SALESEMPLOYEE/SalesEmployee");
const ProjectManager = require("../../models/PROJECTMANAGER/ProjectManager");
...
router.get("/my-meetings", authenticateToken, async (req, res) => {
  try {
    ...
    const meetings = await Meeting.find({ organizer: teamLeaderId })
      .populate('participants.employeeId', 'name email phone')
      .sort({ createdAt: -1 });
...
router.post("/create", authenticateToken, async (req, res) => {
  try {
    ...
    if (!title || !googleMeetLink || !timeSlots || timeSlots.length === 0) {
      return res.status(400).json({
        success: false,
        message: "Title, Google Meet link, and at least one time slot are required",
      });
    }
    ...
    const processedParticipants = await Promise.all(
      participants.map(async (participant) => {
        let employeeModel = 'SalesEmployee'; // Default
        
        // Determine the correct model based on employee type or other criteria
        const salesEmp = await SalesEmployee.findById(participant.employeeId);
        if (salesEmp) {
          employeeModel = 'SalesEmployee';
        } else {
          const pmEmp = await ProjectManager.findById(participant.employeeId);
          if (pmEmp) {
            employeeModel = 'ProjectManager';
          }
    ...
    await savedMeeting.populate('participants.employeeId', 'name email phone');
...
router.put("/:id", authenticateToken, async (req, res) => {
  try {
    ...
    await updatedMeeting.populate('participants.employeeId', 'name email phone');
    await updatedMeeting.populate('organizer', 'name email');
...
// Get meeting calendar (for specific date range)
router.get("/calendar/events", authenticateToken, async (req, res) => {
  try {
    ...
    .select('title timeSlots participants meetingType status');
