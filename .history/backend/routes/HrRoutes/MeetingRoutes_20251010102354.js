...
/**
 * DEBUG: Test HR authentication endpoint
 */
router.get("/debug-auth", authenticateToken, async (req, res) => {
  try {
    console.log("🔍 HR Debug Auth - req.user:", {
      _id: req.user._id,
      name: req.user.name,
      role: req.user.role,
    ...
    console.error("❌ HR Debug Auth error:", error);
    res.status(500).json({
      success: false,
...
router.post("/", authenticateToken, async (req, res) => {
  try {
    ...
    console.log("📥 Received meeting creation data:", {
      title,
      schedule,
      platform,
    ...
    const organizer = {
      ...
      employeeModel: 'HrEmployee', // Reference to HR collection
      name: name,
      email: companyEmail,
    ...
    const meetingData = {
      ...
      createdByModel: 'HrEmployee', // Specific to HR collection
      status: 'scheduled'
    };
