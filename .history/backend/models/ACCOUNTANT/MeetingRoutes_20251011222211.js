...
const { authenticateToken } = require("../../middleware/AccountantAuthMiddleware");

const router = express.Router();

/**
 * GET /api/accountant/meetings/fetch-all - Get all meetings for Accountant
 */
router.get("/fetch-all", authenticateToken, async (req, res) => {
  try {
    const { _id: employeeId, name, role } = req.user;
    
    console.log("📊 Fetching meetings for Accountant:", { employeeId, name, role });
    ...
    // Build filter - Accountant can see meetings where accountant department is invited
    let filter = {
      'invitedDepartments.department': 'accountant',
    ...
    console.log(`✅ Found ${meetings.length} meetings for Accountant`);
    ...
    console.error("❌ Error fetching Accountant meetings:", error);
    res.status(500).json({
      success: false,
...
/**
 * GET /api/accountant/meetings/fetch-stats - Get Accountant meeting statistics
 */
router.get("/fetch-stats", authenticateToken, async (req, res) => {
  try {
    ...
    // Get meetings where this Accountant is invited
    ...
    console.error("❌ Error fetching Accountant meeting stats:", error);
    res.status(500).json({
      success: false,
...
/**
 * GET /api/accountant/meetings/fetch-by-id/:id - Get specific meeting details
 */
...
/**
 * PUT /api/accountant/meetings/update-response/:id - Update meeting response status
 */
