...
const { authenticateToken } = require("../../middleware/ProjectManagerAuthMiddleware");

const router = express.Router();

/**
 * GET /api/project-manager/meetings/fetch-all - Get all meetings for Project Manager
 */
router.get("/fetch-all", authenticateToken, async (req, res) => {
  try {
    const { _id: employeeId, name, role } = req.user;
    
    console.log("📊 Fetching meetings for Project Manager:", { employeeId, name, role });
    ...
    // Build filter - Project Manager can see meetings where project_manager department is invited
    let filter = {
      'invitedDepartments.department': 'project_manager',
    ...
    console.log(`✅ Found ${meetings.length} meetings for Project Manager`);
    ...
    console.error("❌ Error fetching Project Manager meetings:", error);
    res.status(500).json({
      success: false,
...
/**
 * GET /api/project-manager/meetings/fetch-stats - Get Project Manager meeting statistics
 */
router.get("/fetch-stats", authenticateToken, async (req, res) => {
  try {
    ...
    // Get meetings where this Project Manager is invited
    ...
    console.error("❌ Error fetching Project Manager meeting stats:", error);
    res.status(500).json({
      success: false,
...
/**
 * GET /api/project-manager/meetings/fetch-by-id/:id - Get specific meeting details
 */
...
/**
 * PUT /api/project-manager/meetings/update-response/:id - Update meeting response status
 */
router.put("/update-response/:id", authenticateToken, async (req, res) => {
  try {
    ...
    const meeting = await Meeting.findOneAndUpdate(
      ...
      { 
        arrayFilters: [
          { 'dept.department': 'project_manager' },
