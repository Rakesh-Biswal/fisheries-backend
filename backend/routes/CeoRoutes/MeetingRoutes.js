...
// Get all meetings organized by CEO
router.get('/', ceoAuth, async (req, res) => {
  try {
    const { date } = req.query;
    
    let query = { 
      organizer: req.ceo.id,
      organizerModel: 'CEO'
    };
...
// Create new meeting
router.post('/', ceoAuth, async (req, res) => {
  try {
    const {
      title,
      description,
...
// Update meeting
router.put('/:id', ceoAuth, async (req, res) => {
  try {
    const {
      title,
      description,
...
// Delete meeting
router.delete('/:id', ceoAuth, async (req, res) => {
  try {
...
router.get('/stats', ceoAuth, async (req, res) => {
  try {
    ...
    const stats = await Meeting.aggregate([
      { 
        $match: { 
          organizer: new mongoose.Types.ObjectId(req.ceo.id),
          organizerModel: 'CEO'
        } 
      },
      {
